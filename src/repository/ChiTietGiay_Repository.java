...
public class ChiTietGiay_Repository {
    ...
    public static ChiTietGiay_View getAllChiTietGiayViewByMaBarcode(String maBarCode) {
        List<ChiTietGiay_View> listGV = new ArrayList<>();
        ChiTietGiay_View ctg = null;
        String sql = "select maGiay,tenGiay,maCode, tenMauSac, Size, tenChatLieu, tenHangGiay, namBaoHanh, soLuong, giaNhap ,giaBan, anh\n"
                + "from ChiTietGiay join MauSac on ChiTietGiay.idMauSac=MauSac.id\n"
                + "				 join Size on ChiTietGiay.idSize=Size.id\n"
                + "				 join ChatLieu on ChiTietGiay.idChatLieu=ChatLieu.id\n"
                + "				 join HangGiay on ChiTietGiay.idHangGiay=HangGiay.id where maCode=?";
        ResultSet rs = JDBC_HELPER.selectTongQuat(sql, maBarCode);
        try {
            while (rs.next()) {
                String maG = rs.getString("maGiay");
                String tenG = rs.getString("tenGiay");
    ...
    public static boolean checkTrungMa(String maGiay) {
        ChiTietGiay_View gv = getAllChiTietGiayViewByMa(maGiay);
        if (gv == null) {
            return false;
        } else {
            return true;
    ...
    public static boolean checkTrungMaBarcode(String maCode) {
        ChiTietGiay_View gv = getAllChiTietGiayViewByMaBarcode(maCode);
        if (gv == null) {
            return false;
        } else {
            return true;
