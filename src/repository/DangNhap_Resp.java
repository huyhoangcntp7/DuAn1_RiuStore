...
public class DangNhap_Resp {
    ...
    public static DangNhap_ViewModel getThongTinChuCH(String email, String mk) {
        DangNhap_ViewModel dn = null;
        String sql = "select * from NhanVien inner join chucvu on nhanvien.idChucvu= chucvu.id where tenchucvu=N'Chủ cửa hàng' and email=? and matkhau=?";
        ResultSet rs = JDBC_HELPER.selectTongQuat(sql, email, mk);
        try {
            while (rs.next()) {
                dn = new DangNhap_ViewModel();
    ...
    public static DangNhap_ViewModel getThongTinChuByTen(String ten) {
        DangNhap_ViewModel dn = null;
        String sql = "select * from NhanVien inner join chucvu on nhanvien.idChucvu= chucvu.id where tenchucvu=N'Chủ cửa hàng' and hoTenNhanVien =?";
    ...
    public static boolean checkDNChuCh(String email, String mk) {

        if (getThongTinChuCH(email, mk) == null) {
            return false;
        }
        return true;
    }
    public static boolean checkDNChuChQuyen(String ten) {
       
        if (getThongTinChuByTen(ten) == null) {
            return false;
        }
        return true;
    }
